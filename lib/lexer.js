  ...
  exports.Lexer = Lexer = (function() {
    ...
    Lexer.prototype.tokenize = function(code, opts) {
      ...
      while (c = code.charAt(i)) {
        ...
        switch (c) {
          case ' ':
            i += this.whitespaceToken(i);
            break;
          case '\t':
            i += this.whitespaceToken(i);
            break;
          case '#':
            i += this.commentToken(i);
            break;
          case '`':
            i += this.jsToken(i);
            break;
          case '"':
            i += this.heredocToken(i) || this.stringToken(i);
            break;
          case '\'':
            i += this.heredocToken(i) || this.stringToken(i);
            break;
          case '/':
            i += this.regexToken(i) || this.literalToken(i);
            break;
          default:
            i += this.identifierToken(i) || this.commentToken(i) || this.whitespaceToken(i) || this.lineToken(i) || this.numberToken(i) || this.literalToken(i);
        }
      }
    ...
    Lexer.prototype.stringToken = function(lastIndex) {
      ...
      switch (this.code.charAt(lastIndex)) {
        case "'":
          ...
          break;
        case '"':
          if (!(string = this.balancedString(this.code.substr(lastIndex), '"'))) {
            return 0;
          }
    ...
    Lexer.prototype.commentToken = function(lastIndex) {
      ...
      if (!(match = this.code.match(COMMENT))) {
        return 0;
      }
    ...
    Lexer.prototype.jsToken = function(lastIndex) {
      var match, script;
      if (!(this.code.charAt(lastIndex) === '`' && (match = JSTOKEN.exec(this.code)))) {
        return 0;
      }
    ...
    Lexer.prototype.whitespaceToken = function(lastIndex) {
      ...
      if (!((match = WHITESPACE.exec(this.code)) || (nline = this.code.charAt(lastIndex) === '\n'))) {
        return 0;
      }
      prev = last(this.tokens);
      if (prev) {
    ...
    Lexer.prototype.literalToken = function(lastIndex) {
      ...
      if (match = OPERATOR.exec(this.code)) {
        value = match[0];
        if (CODE.test(value)) {
          this.tagParameters();
        }
    ...
    Lexer.prototype.unfinished = function(lastIndex) {
      ...
      return LINE_CONTINUER.test(this.code) || (prev = last(this.tokens, 1)) && prev[0] !== '.' && (value = this.value()) && !value.reserved && NO_NEWLINE.test(value) && !CODE.test(value) && !ASSIGNED.test(this.code);
    };
